...
static void sha512(__generic ulong *input, const uint length, __generic ulong *hash);
static void sha256(__generic const uint *pass, int pass_len, __generic uint* hash);
static void hmac_sha512(const __generic uchar* key, uint key_len, const __generic uchar* data, uint data_len, __generic uchar* output);
static void sha512_gpu(const __generic uchar* data, uint len, __generic uchar* hash);
static void sha512_compress(__generic ulong *State, __generic const ulong *block);
static void sha256_local(__local uint * restrict workspace, const uint length, __generic uint * restrict hash);
static void sha512_local(__local ulong * restrict workspace, const uint length, __generic ulong * restrict hash);
...
static int md_pad_128(__generic ulong *msg, const long msgLen_bytes) {
...
static void sha256_process2 (const __generic uint *W, __generic uint *digest) {
  uint a = digest[0];
  uint b = digest[1];
  uint c = digest[2];
  uint d = digest[3];
...
static void hmac_sha512(
    const __generic uchar* key, uint key_len,
    const __generic uchar* data, uint data_len,
    __generic uchar* output
) {
    ulong ipad_block[16];
    ulong opad_block[16];
    ...
    // HMAC spec: if key > block size (128 bytes), pre-hash it with SHA-512 to 64 bytes.
    // BUG FIX: 24-word BIP39 mnemonics are ~155 chars — they MUST be hashed here,
    // otherwise they get silently truncated to 128 bytes, producing the wrong seed.
    uchar key_buf[128] = {0};
    if (key_len > 128) {
        // Pre-hash key per HMAC spec (RFC 2104 section 2)
        uchar hashed_key[64];
        sha512_gpu(key, key_len, hashed_key);
        for(int i=0; i<64; i++) key_buf[i] = hashed_key[i];
        // key_len is now effectively 64 — key_buf already zero-padded to 128
    } else {
        for(uint i=0; i<key_len; i++) key_buf[i] = key[i];
    }
...
static void sha256_local(
    ...
    sha256((__generic const uint*)private_input, length, hash);
}
...
static void sha512_local(
    ...
    sha512((__generic ulong*)private_input, length, hash);
}