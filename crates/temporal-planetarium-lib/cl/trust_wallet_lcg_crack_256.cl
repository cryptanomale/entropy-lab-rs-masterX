...
//
// 8 combos: VariantAnd/Mod x m/44', m/84', m/49', m/86'(P2TR)
...
static void lcg_entropy_and_256(uint seed, __private uchar entropy[32]) {
    ...
    for (int i = 0; i < 32; i++) { s = minstd_next_256(s); entropy[i] = (uchar)(s & 0xFFu); }
}

static void lcg_entropy_mod_256(uint seed, __private uchar entropy[32]) {
    uint s = minstd_seed_256(seed);
    for (int i = 0; i < 32; i++) { s = minstd_next_256(s); entropy[i] = (uchar)(s % 255u); }
}
