...
int module_hash_decode (MAYBE_UNUSED const hashconfig_t *hashconfig, MAYBE_UNUSED void *digest_buf, MAYBE_UNUSED salt_t *salt, MAYBE_UNUSED void *esalt_buf, MAYBE_UNUSED void *hook_salt_buf, MAYBE_UNUSED hashinfo_t *hash_info, const char *line_buf, MAYBE_UNUSED const int line_len)
{
  ...
  // Tokenize the input: $milksad$<purpose>$<timestamp>$<address>
  hc_token_t token;
  ...
  if (purpose == 44 && address_pos[0] == '1')
  {
    ...
    decode_success = base58_decode_address (address_pos, address_len, 0x00, milksad->target_hash160);
  }
  else if (purpose == 49 && address_pos[0] == '3')
  {
    // P2SH - decode Base58, expect version 0x05
    decode_success = base58_decode_address (address_pos, address_len, 0x05, milksad->target_hash160);
  }
  else if (purpose == 84 && address_pos[0] == 'b' && address_pos[1] == 'c' && address_pos[2] == '1')
  {
    // Bech32 - decode Bech32
    decode_success = bech32_decode_address (address_pos, address_len, milksad->target_hash160);
  }
  else
  {
